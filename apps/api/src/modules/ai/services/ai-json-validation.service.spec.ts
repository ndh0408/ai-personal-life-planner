...
describe('AiJsonValidationService', () => {
  it('parses well-formed JSON', async () => {
    const service = new AiJsonValidationService(new AiProviderService(new MockAiProvider(), makeStubUsage()));
  ...
  it('repairs once when the first response is malformed', async () => {
    ...
    const service = new AiJsonValidationService(new AiProviderService(provider, makeStubUsage()));
